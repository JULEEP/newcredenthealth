...
import { 
  ...
  Package, TestTube, Filter, Search, Shield, ChevronDown, ChevronUp,
  ShoppingBag, ArrowLeft, Clock as ClockIcon, DollarSign, Info, AlertCircle,
  Star, Heart, Brain, Eye, Droplets, Scan
} from "lucide-react";
...
const StaffPackageBookingPage = () => {
  ...
  const [error, setError] = useState(""); // ✅ Yeh line add karo

  useEffect(() => {
    // Get package data from navigation
    if (location.state) {
      setSelectedPackage({
  ...
  useEffect(() => {
    filterDiagnostics();
  }, [diagnostics, selectedCityFilter]);
 ...
 const fetchCompanyDiagnostics = async () => {
  ...
  // ✅ Staff ID get karein localStorage se
  const staffId = localStorage.getItem("staffId");
  ...
  console.log("📡 Fetching diagnostics for:", { companyId, staffId });
  
  try {
    // ✅ ONLY new API with both companyId and staffId
    ...
    if (response.data && response.data.data) {
      // ✅ Filter karein sirf woh diagnostics jo packages rakhte hain
      const diagnosticsWithPackages = response.data.data.filter(
        diagnostic => diagnostic.packages && diagnostic.packages.length > 0
      );
      
      console.log(`📦 Found ${diagnosticsWithPackages.length} diagnostics with packages`);
      
      setDiagnostics(diagnosticsWithPackages);
      setFilteredDiagnostics(diagnosticsWithPackages);
      
      // ✅ Show info about staff's packages if available
      if (response.data.staffPackageInfo) {
        const { totalMyPackages, hasPackages } = response.data.staffPackageInfo;
        console.log(`🛒 Staff has ${totalMyPackages} packages in myPackages array`);
        
        if (hasPackages && diagnosticsWithPackages.length === 0) {
          setError("Your packages are not available in any diagnostic center");
        }
      }
    ...
    // ✅ ONLY new API errors handle karein
    if (err.response) {
  ...
  const filterDiagnostics = () => {
    ...
    if (selectedCityFilter !== "all" && selectedCityFilter) {
      filtered = filtered.filter(diagnostic => 
        diagnostic.city?.toLowerCase().includes(selectedCityFilter.toLowerCase()) ||
        diagnostic.address?.toLowerCase().includes(selectedCityFilter.toLowerCase())
      );
    }
    ...
    try {
      ...
      if (availableBalance >= packagePrice) {
        ...
        if (response.data.isSuccessfull) {
          alert("✅ Package Booking Successful with Wallet!");
          handlePopupClose();
          navigate('/mybookings');
        } else {
      ...
      handler: async function (response) {
        ...
        try {
          ...
          if (bookingResponse.data.isSuccessfull) {
            alert("✅ Package Booking Successful after Payment!");
            handlePopupClose();
            navigate('/mybookings');
          } else {
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="text-3xl font-bold mb-2">Book Your Package</h1>
              <div className="flex flex-wrap items-center gap-4">
                <div className="bg-white/20 px-4 py-2 rounded-lg">
                  <h2 className="text-xl font-semibold">{selectedPackage.packageName}</h2>
                </div>
      ...
      {/* Main Content - ONLY DIAGNOSTIC CENTERS */}
        ...
        <div className="container mx-auto px-4">
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 text-lg">No diagnostic centers available in selected city.</p>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredDiagnostics.map((diagnostic) => (
                <div key={diagnostic._id} className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 group">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center text-gray-600 text-sm">
                          ...
                          <span>{diagnostic.city || diagnostic.address?.split(",")[0]}</span>
                        </div>
                      </div>
                    </div>
                    <div className={`p-2 rounded-lg ${getDiagnosticColor(diagnostic.centerType)}`}>
                      <span className="text-xl">{getDiagnosticIcon(diagnostic.centerType)}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center text-gray-600 text-sm">
                      ...
                      <span>{diagnostic.phone || 'Contact not available'}</span>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-gradient-to-r from-blue-500 to-blue-900 text-white font-medium rounded-xl hover:from-blue-600 hover:to-blue-600 shadow-lg hover:shadow-xl transition-all duration-300"
                  >
                    Select This Center
                  </button>
                </div>
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-70 flex justify-center items-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-gray-600">{selectedDiagnostic.name}</span>
            ...
            <div className="p-6">
              ...
              <div className="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-5 border border-blue-200">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h4 className="font-bold text-gray-900 text-lg mb-2">{selectedPackage.packageName}</h4>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center">
                        <TestTube className="w-4 h-4 text-blue-600 mr-2" />
                        <span className="text-sm text-gray-700">Health Package</span>
                      </div>
                      <div className="flex items-center">
                        <DollarSign className="w-4 h-4 text-green-600 mr-2" />
                        <span className="text-lg font-bold text-green-600">₹{selectedPackage.price || 0}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right ml-4">
                    <div className="bg-white px-3 py-1 rounded-lg shadow">
                      <span className="text-sm font-medium text-blue-600">Package Booking</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {!selectedOption && (
                <div className="mb-8">
                  <h4 className="text-lg font-semibold text-gray-800 mb-4">Select Service Type</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button
                      onClick={() => handleServiceSelect("Home Collection")}
                      className="p-5 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-100 rounded-xl flex items-center transition-all duration-300 hover:from-green-100 hover:to-emerald-100 hover:border-green-200 hover:shadow-lg group"
                    >
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-sm text-gray-600 mt-1">Sample collection at your doorsteps</p>
                      </div>
                    </button>

                    <button
                      onClick={() => handleServiceSelect("Center Visit")}
                      className="p-5 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-100 rounded-xl flex items-center transition-all duration-300 hover:from-blue-100 hover:to-indigo-100 hover:border-blue-200 hover:shadow-lg group"
                    >
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-sm text-gray-600 mt-1">Visit our diagnostic center</p>
                      </div>
                    </button>
                  </div>
                </div>
              ...
              {selectedOption && (
                <div className="mb-8">
                  <h4 className="text-lg font-semibold text-gray-800 mb-4">Choose Date</h4>
                  {availableDates.length > 0 ? (
                    <div className="flex flex-wrap gap-3">
                      {availableDates.slice(0, 5).map((date, index) => (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center w-24 h-24 rounded-xl text-sm relative transition-all duration-300 ${selectedDate === date
                        ...
                        >
                          ...
                          <span className="text-2xl font-bold mt-1">{formatDate(date).split(' ')[1]}</span>
                    ...
                    <div className="text-center py-6 bg-gray-50 rounded-xl">
                      ...
                      <p className="text-gray-600">No available dates found for this service type.</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {selectedDate && selectedOption && (
                <div className="mb-8">
                  <h4 className="text-lg font-semibold text-gray-800 mb-4">Choose Time Slot</h4>
              ...
              {selectedDate && selectedTime && (
                <div className="mb-8">
                  ...
                  <div className="grid gap-3 max-h-64 overflow-y-auto pr-2">
                    ...
                    {/* Map the rest of the family members */}
                ...
                <div className="pt-6 border-t border-gray-200">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-green-600">₹{selectedPackage.price || 0}</p>
                      {walletData && (
                    ...
                    <button
                      ...
                      className={`px-8 py-3.5 font-semibold rounded-xl transition-all duration-300 flex items-center justify-center ${processingPayment || isBookDisabled()
